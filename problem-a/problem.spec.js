...
describe("Includes the polyfills in the HTML", () => {
  test("loaded both polyfills", () => {
    ...
    expect(scriptSrcs).toEqual(
      expect.arrayContaining([expect.stringMatching(/fetch(\.min)?\.js/)])
    ); //expect fetch to be in the scripts
    expect(scriptSrcs).toEqual(
      expect.arrayContaining([
        expect.stringMatching(/es6-promise(\.auto)?(\.min)?\.js/),
      ])
    ); //expect promise to be in the scripts
  });
});
...
describe("Song searching page", () => {
  ...
  test("renders individual tracks", () => {
    ...
    expect(img.attr("src")).toEqual(
      solution.EXAMPLE_SEARCH_RESULTS.results[1].artworkUrl100
    ); //has correct artwork
    let trackRegex = new RegExp(
      solution.EXAMPLE_SEARCH_RESULTS.results[1].trackName.replace(
        /[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,
        "\\$&"
      )
    ); //get name, escape as regex
  ...
  test("renders search results", () => {
    ...
    expect(records.eq(0).attr("src")).toEqual(
      solution.EXAMPLE_SEARCH_RESULTS.results[0].artworkUrl100
    ); //should all be different
    expect(records.eq(1).attr("src")).toEqual(
      solution.EXAMPLE_SEARCH_RESULTS.results[1].artworkUrl100
    );
    expect(records.eq(2).attr("src")).toEqual(
      solution.EXAMPLE_SEARCH_RESULTS.results[2].artworkUrl100
    );
  ...
  test("fetches track information and renders the result", async () => {
    ...
    expect(records.eq(1).attr("src")).toEqual(
      solution.EXAMPLE_SEARCH_RESULTS.results[1].artworkUrl100
    ); //one of them matches the (sample) fetched
  });
  ...
  test("searches on form submission", (done) => {
    ...
    setImmediate(() => {
      //wait until promise has resolved (one tick)
